...
import { ROLE_INFO, type GameRole } from "@/lib/game/roles";
...
export function JoinGameModal({ isOpen, onClose, prefillCode }: JoinGameModalProps) {
  ...
  const getAvailableRoles = (): GameRole[] => {
    ...
    let baseRoles: GameRole[];
    
    if (gameInfo.game_mode === "single_player") {
      baseRoles = ["player_a"];
    } else if (gameInfo.game_mode === "two_player") {
      // For 2-player games with human GM: player_a + game_master
      // For 2-player games with AI GM: player_a + player_b
      baseRoles = gameInfo.game_master === "player"
        ? ["player_a", "game_master"]
        : ["player_a", "player_b"];
    } else {
      // multi_player: all roles, but game_master only for human GM games
      baseRoles = gameInfo.game_master === "player"
        ? ["player_a", "player_b", "game_master"]
        : ["player_a", "player_b"];
    }
    
    return baseRoles.filter((role) => isRoleAvailable(role));
  };
