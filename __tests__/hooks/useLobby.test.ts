...
describe("useLobby", () => {
  ...
  describe("setPlayerReady", () => {
    it("should update player status to ready", async () => {
      mockQuery.eq.mockResolvedValueOnce({ data: mockPlayers, error: null });
      ...
      expect(mockSupabase.from).toHaveBeenCalledWith("players");
      expect(mockQuery.update).toHaveBeenCalledWith({ status: "ready" });
    });

    it("should update player status to waiting", async () => {
      mockQuery.eq.mockResolvedValueOnce({ data: mockPlayers, error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: null });
      ...
      expect(mockQuery.update).toHaveBeenCalledWith({ status: "waiting" });
    });
    ...
    it("should throw error on failure", async () => {
      ...
      ).rejects.toThrow("Failed to update ready status");
  ...
  describe("kickPlayer", () => {
    it("should delete player from game", async () => {
      mockQuery.eq.mockResolvedValueOnce({ data: mockPlayers, error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: null });
    ...
    it("should throw error on failure", async () => {
      mockQuery.eq.mockResolvedValueOnce({ data: mockPlayers, error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: { message: "Delete failed" } });
      ...
      ).rejects.toThrow("Failed to kick player");
  ...
  describe("startGame", () => {
    it("should update game status to ready", async () => {
      mockQuery.eq.mockResolvedValueOnce({ data: mockPlayers, error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: null });
    ...
    it("should update all players to playing status", async () => {
      mockQuery.eq.mockResolvedValueOnce({ data: mockPlayers, error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: null });
      mockQuery.eq.mockResolvedValueOnce({ error: null });
      ...
      expect(mockSupabase.from).toHaveBeenCalledWith("players");
      expect(mockQuery.update).toHaveBeenCalledWith({ status: "playing" });
    });
    ...
    it("should throw error if game update fails", async () => {
      ...
      ).rejects.toThrow("Failed to start game");
